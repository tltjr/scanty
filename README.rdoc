...
something other than the default. The rest of the Blog config struct configures
the navigation links at the top of the blog. They links are optional and can be
toggled off by setting :main_nav = false. If they are left on, change the name
fields to what you would like displayed on your blog and the link fields to
where you want to direct users. See the comments with the struct and 
http://thomasthornton.heroku.com/ for an example.
